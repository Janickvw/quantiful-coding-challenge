...
const ExchangeRateCard = props => (
  <Card>
    <CardContent>
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            {Object.keys(props.rates).map(currency => (
              <TableCell key={`exchangeCurrency_${currency}`}>
                {currency}
              </TableCell>
            ))}
          </TableRow>
        ...
        <TableBody>
          <TableRow>
            ...
            {Object.values(props.rates).map((rate, i) => (
              <TableCell key={`exchangeRate_${i}`}>{rate}</TableCell>
            ))}
          </TableRow>
        </TableBody>
      </Table>
      <Typography>
        {" "}
        <em> Note: This means that 1NZD = 0.95AUD </em>{" "}
      </Typography>
    </CardContent>
  </Card>
);
